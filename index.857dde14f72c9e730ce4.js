...
document.addEventListener('DOMContentLoaded', function () {
  var grids = document.querySelectorAll('.grid--1');
  grids.forEach(function (grid) {
    var items = grid.querySelectorAll('.grid__item');
    items.forEach(function (item, i) {
      item.style.gridArea = "ga".concat(i);
    });
  });
  var icons = document.querySelectorAll('.icon');
  var updatePositions = function updatePositions(iconCount) {
    icons.forEach(function (icon, index) {
      var angle = 360 / iconCount * index;
      icon.style.setProperty('--angle', "".concat(angle, "deg"));
      /* И снова вращаем  */
    });
  };
  ...
  new (sticky_header_cjs_default())({
    ...
    scrollSettings: {
      ...
      breakPosition: 125 // Hide position
    }
  });
  var blocks = document.querySelectorAll('.block-v-animate');
  ...
  var R = 420;
  document.querySelectorAll(".child-wrapper").forEach(function (item, i, items) {
    var parent = item.parentElement;
    var x = Math.round(parent.clientWidth / 2);
    var y = Math.round(parent.clientHeight / 2);
    item.style.top = y + R * Math.sin(i * 2 * Math.PI / items.length) + "px";
    item.style.left = x + R * Math.cos(i * 2 * Math.PI / items.length) + "px";
  });

  // function showServices() {
  // 	const btnShow = document.querySelector('#showServices')
  ...
  (function () {
    ...
    var mStatus = false;
    var _iterator = _createForOfIteratorHelper(mOpen),
      _step;
    try {
    ...
    var _iterator2 = _createForOfIteratorHelper(mClose),
      _step2;
    try {
